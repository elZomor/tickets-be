...
def upload_to_arabic_festival_show(instance, filename):
    instance_id = instance.id or "unassigned"
    cast_name = slugify("unknown")
    created_at = datetime.now().strftime('%Y-%m-%d')
    ext = os.path.splitext(filename)[1]
    file_name = f"{instance_id}_{cast_name}_{created_at}{ext}"
    return os.path.join('media', 'arabic_festival_shows', 'poster', file_name)


def upload_to_arabic_festival(instance, filename):
    instance_id = instance.id or "unassigned"
    created_at = datetime.now().strftime('%Y-%m-%d')
    file_name = f"{instance_id} - {created_at}{os.path.splitext(filename)[1]}"
    return os.path.join('media', 'arabic_festival', 'poster', file_name)


def upload_to_arabic_festival_article_attachment(instance, filename):
    instance_id = instance.id or "unassigned"
    created_at = datetime.now().strftime('%Y-%m-%d')
    file_name = f"{instance_id} - {created_at}{os.path.splitext(filename)[1]}"
...
def upload_to_alt_spaces_festival(instance, filename):
    instance_id = instance.id or "unassigned"
    created_at = datetime.now().strftime('%Y-%m-%d')
    file_name = f"{instance_id} - {created_at}{os.path.splitext(filename)[1]}"
    return os.path.join('media', 'alt_spaces_festival', 'poster', file_name)


def upload_to_alt_spaces_festival_show(instance, filename):
    instance_id = instance.id or "unassigned"
    cast_name = slugify("unknown")
    created_at = datetime.now().strftime('%Y-%m-%d')
    ext = os.path.splitext(filename)[1]
    file_name = f"{instance_id}_{cast_name}_{created_at}{ext}"
    return os.path.join('media', 'alt_spaces_festival_shows', 'poster', file_name)


def upload_to_alt_spaces_festival_article_attachment(instance, filename):
    instance_id = instance.id or "unassigned"
    created_at = datetime.now().strftime('%Y-%m-%d')
    file_name = f"{instance_id} - {created_at}{os.path.splitext(filename)[1]}"
